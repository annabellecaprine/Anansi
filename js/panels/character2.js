/*
 * Anansi Panel: Character v2 (Dual-Mode: Solo / Ensemble)
 * File: js/panels/character2.js
 * Category: Seeds
...
Use sparingly for pacing. Format: *Narrator: [description]*`;
  ...
  function getFirstMessageOptions(actorIds, state) {
    ...
    actors.forEach(actor => {
      const greetings = Array.isArray(actor.firstMessage)
        ? actor.firstMessage
        : (actor.firstMessage ? [actor.firstMessage] : []);

      greetings.forEach((greeting, idx) => {
        if (greeting && greeting.trim()) {
          options.push({
            label: `${actor.name || 'Actor'}${greetings.length > 1 ? ` (${idx + 1})` : ''}`,
            content: greeting,
            actorId: actor.id
          });
        }
    ...
    // Always add Custom (blank) option at the end
    options.push({
      label: 'Custom',
      content: '',
      actorId: null,
